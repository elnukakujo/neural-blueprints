...
class FeatureTabularSingleLabelDataset(TabularSingleLabelDataset):
    """
    PyTorch Dataset for tabular data with single label classification/regression that includes feature representations generated by a representation model.
    ...
    def __init__(
            self,
            column_encoders: dict[str | tuple[str, ...], EncoderArchitecture],
            data: pd.DataFrame,
            discrete_features: list[str],
            continuous_features: list[str],
            label_column: str,
        ...
        for column_to_encode, representation_model in column_encoders.items():
            if isinstance(column_to_encode, str):
                cols = [column_to_encode]
                key = column_to_encode
            else:
                cols = list(column_to_encode)
                key = "-".join(column_to_encode)
            ...
            with torch.no_grad():
                self.x[key] = representation_model.encode(data_to_encode).detach()

        columns_not_encoded = [col for col in self.columns if col not in column_encoders]
        columns_not_encoded.remove(label_column)
        self.x["passthrough"] = self.data[:, [self.columns.index(col) for col in columns_not_encoded]]
    
    def __getitem__(self, idx):
        row = {key: value[idx] for key, value in self.x.items()}
        label = self.labels[idx]
        return row, label
